...
| `knowledge-diffusion_silos`          | 0.156                            | 0.154                  | **obsolete by intent** — the chart now ranks its 30 files by silo risk (size × churn × sole ownership × recency) instead of by editor count tie-broken alphabetically, so it plots a different file set from Python's and the recorded RMSE no longer describes anything. Deliberate deviation; see PLAN.md T3.2 |
