...
- [ ] Relicense the labours-go code under Apache-2.0 (add license headers / NOTICE) to match this repo. -> not needed, I'm code owner of both repos and can relicense.
- [ ] `diff labours-go/pb.proto internal/pb/pb.proto`; record any drift and reconcile to
      `internal/pb/pb.proto` as the single source of truth.
- [ ] Inventory the render stack: confirm each `cwbudde/*` module builds standalone and decide its tag.

### Phase 1 — Publish/tag the render stack
- [ ] Tag real versions of `matplotlib-go` (currently v0.0.0), `agg_go`, `algo-fft`, `mathtext`,
      `qhull-go`; verify each builds with no sibling `replace`. Prerequisite for de-replacing.
- [ ] Acceptance: `go build` of `labours-go` succeeds against the tagged versions (temporarily
